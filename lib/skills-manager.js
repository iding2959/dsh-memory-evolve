...
export function installSkillsManager(ctx, options = {}) {
  ...
  ctx.inject(['skills'], (skillCtx) => {
    ...
    // 技能管理会看到空列表。这里取默认 preset 的视图 scope 作为所有目录查询的
    // 作用域：agentPresets 服务缺失（旧快照/TUI/测试）时回退 undefined，行为
    // 与本修复前一致。
    // 0.1.7：standingKeyFor() 随 preset 重写删除，公开面只剩带租约的
    // acquireScope()。租约必须每次读取后释放——缓存 key 会钉住已退役的 preset
    // 修订（旧技能树不释放，视图停在旧层），所以 scope 只作为回调参数存在。
    const withScope = async (read) => {
      const presets = skillCtx.get('agentPresets')
      if (!presets || typeof presets.acquireScope !== 'function') return read(undefined)
      let lease
      try {
    ...
    const reconcileOnce = async () => {
      ...
      try {
        ...
        list = await withScope((scope) => skillCtx.skills.list({ ...(scope !== undefined ? { scope } : {}) }))
      } catch (error) {
    ...
    const addCustomDir = async (input) => {
      ...
      let roots = []
      try {
        ({ roots } = await withScope((scope) => collectRoots(skillCtx, undefined, scope)))
      } catch {
        roots = []
      }
      for (const root of roots) {
      ...
      const handler = async (req, res) => {
        ...
        if (pathname === '/skills-manager/api/skills' && req.method === 'GET') {
          try {
            ...
            const { skills, roots } = await withScope((scope) => collectRoots(webCtx, cwd, scope))
            sendJson(res, 200, {
        ...
        if (pathname === '/skills-manager/api/browse' && req.method === 'GET') {
          try {
            const { roots } = await withScope((scope) => collectRoots(webCtx, resolveCwdFor(query), scope))
